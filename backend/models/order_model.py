...
class OrderItem(BaseModel):
    ...
    subtotal: float = Field(
        ..., gt=0, description="Total price for this item (price * quantity)"
    )
    ...
    def validate_subtotal(cls, v, info):
        ...
        if "price" in info.data and "quantity" in info.data:
            ...
            if (
                ...
                raise ValueError(
                    f"Subtotal {v} does not match price * quantity = {expected_subtotal}"
                )
        return v
...
class DeliveryAddress(BaseModel):
    ...
    instructions: Optional[str] = Field(
        None, max_length=500, description="Delivery instructions"
    )
...
class OrderCreate(BaseModel):
    ...
    def validate_order(self):
        ...
        if abs(self.subtotal - calculated_subtotal) > 0.01:
            raise ValueError(
                f"Subtotal {self.subtotal} does not match sum of item subtotals {calculated_subtotal}"
            )
        ...
        if abs(self.total_amount - expected_total) > 0.01:
            raise ValueError(
                f"Total amount {self.total_amount} does not match calculated total {expected_total}"
            )
...
class Order(OrderCreate):
    ...
    delivery_user_id: Optional[str] = Field(
        None, description="ID of the delivery user (if assigned)"
    )
    status: OrderStatus = Field(
        OrderStatus.PENDING, description="Current status of the order"
    )

    # Timing fields
    estimated_pickup_time: Optional[datetime] = Field(
        None, description="Estimated time for pickup"
    )
    estimated_delivery_time: Optional[datetime] = Field(
        None, description="Estimated time for delivery"
    )
    actual_pickup_time: Optional[datetime] = Field(
        None, description="Actual time when order was picked up"
    )
    actual_delivery_time: Optional[datetime] = Field(
        None, description="Actual time when order was delivered"
    )

    # Metadata
    created_at: Optional[datetime] = Field(
        None, description="When the order was created"
    )
    updated_at: Optional[datetime] = Field(
        None, description="When the order was last updated"
    )
    # Delivery verification fields
    delivery_code: Optional[str] = Field(
        None, description="Numeric delivery verification code shown to customer"
    )
    delivery_code_used: Optional[bool] = Field(
        False, description="Whether the delivery code has been used"
    )
...
class OrderUpdate(BaseModel):
    ...
    delivery_user_id: Optional[str] = Field(
        None, description="ID of the delivery user to assign"
    )
    estimated_pickup_time: Optional[datetime] = Field(
        None, description="Updated estimated pickup time"
    )
    estimated_delivery_time: Optional[datetime] = Field(
        None, description="Updated estimated delivery time"
    )
    actual_pickup_time: Optional[datetime] = Field(
        None, description="Actual pickup time"
    )
    actual_delivery_time: Optional[datetime] = Field(
        None, description="Actual delivery time"
    )
