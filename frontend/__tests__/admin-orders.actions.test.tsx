...
describe('AdminOrdersPage actions', () => {
  beforeEach(() => {
    ...
    (global as any).fetch = jest.fn(
      (input: RequestInfo, opts?: RequestInit) => {
  ...
  it('accepts pending order and updates UI', async () => {
    ...
    const patchCall = calls.find(
      (c: any[]) =>
        String(c[0]).includes('/status') && c[1] && c[1].method === 'PATCH'
    );
  ...
  it('marks confirmed order ready and navigates to restaurant', async () => {
    ...
    const markReadyBtn = await screen.findByRole('button', {
      name: /Mark Ready/i,
    });
